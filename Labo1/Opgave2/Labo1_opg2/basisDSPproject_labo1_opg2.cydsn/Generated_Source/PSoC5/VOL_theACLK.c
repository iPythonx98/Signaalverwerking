...
/*******************************************************************************
* Function Name: VOL_theACLK_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void VOL_theACLK_Start(void) 
{
    /* Set the bit to enable the clock. */
    VOL_theACLK_CLKEN |= VOL_theACLK_CLKEN_MASK;
	VOL_theACLK_CLKSTBY |= VOL_theACLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: VOL_theACLK_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void VOL_theACLK_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    VOL_theACLK_CLKEN &= (uint8)(~VOL_theACLK_CLKEN_MASK);
	VOL_theACLK_CLKSTBY &= (uint8)(~VOL_theACLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: VOL_theACLK_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void VOL_theACLK_StopBlock(void) 
{
    if ((VOL_theACLK_CLKEN & VOL_theACLK_CLKEN_MASK) != 0u)
    {
...
#if defined(VOL_theACLK__CFG3)
        CLK_DIST_AMASK = VOL_theACLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = VOL_theACLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* VOL_theACLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(VOL_theACLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(VOL_theACLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: VOL_theACLK_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void VOL_theACLK_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        VOL_theACLK_CLKSTBY &= (uint8)(~VOL_theACLK_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: VOL_theACLK_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void VOL_theACLK_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = VOL_theACLK_GetSourceRegister();
    uint16 oldDivider = VOL_theACLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = VOL_theACLK_CLKEN & VOL_theACLK_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((VOL_theACLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: VOL_theACLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 VOL_theACLK_GetDividerRegister(void) 
{
    return CY_GET_REG16(VOL_theACLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: VOL_theACLK_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void VOL_theACLK_SetModeRegister(uint8 modeBitMask) 
{
    VOL_theACLK_MOD_SRC |= modeBitMask & (uint8)VOL_theACLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: VOL_theACLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void VOL_theACLK_ClearModeRegister(uint8 modeBitMask) 
{
    VOL_theACLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(VOL_theACLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: VOL_theACLK_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 VOL_theACLK_GetModeRegister(void) 
{
    return VOL_theACLK_MOD_SRC & (uint8)(VOL_theACLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: VOL_theACLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void VOL_theACLK_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = VOL_theACLK_GetDividerRegister();
    uint8 oldSrc = VOL_theACLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        VOL_theACLK_MOD_SRC |= CYCLK_SSS;
        VOL_theACLK_MOD_SRC =
            (VOL_theACLK_MOD_SRC & (uint8)(~VOL_theACLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: VOL_theACLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 VOL_theACLK_GetSourceRegister(void) 
{
    return VOL_theACLK_MOD_SRC & VOL_theACLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: VOL_theACLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void VOL_theACLK_SetPhaseRegister(uint8 clkPhase) 
{
    VOL_theACLK_PHASE = clkPhase & VOL_theACLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: VOL_theACLK_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 VOL_theACLK_GetPhaseRegister(void) 
{
    return VOL_theACLK_PHASE & VOL_theACLK_PHASE_MASK;
}
