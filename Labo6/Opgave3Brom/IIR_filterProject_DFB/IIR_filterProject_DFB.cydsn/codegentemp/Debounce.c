...
void Debounce_Start(void) 
{
    ...
    Debounce_CLKEN |= Debounce_CLKEN_MASK;
	Debounce_CLKSTBY |= Debounce_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Debounce_Stop
********************************************************************************
*
* Summary:
...
void Debounce_Stop(void) 
{
    ...
    Debounce_CLKEN &= (uint8)(~Debounce_CLKEN_MASK);
	Debounce_CLKSTBY &= (uint8)(~Debounce_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Debounce_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Debounce_CLKEN & Debounce_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Debounce_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Debounce_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Debounce__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Debounce_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Debounce_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Debounce_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Debounce_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Debounce_GetSourceRegister();
    uint16 oldDivider = Debounce_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Debounce_CLKEN & Debounce_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Debounce_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Debounce_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Debounce_DIV_PTR);
}


/*******************************************************************************
* Function Name: Debounce_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Debounce_MOD_SRC |= modeBitMask & (uint8)Debounce_MODE_MASK;
}


/*******************************************************************************
* Function Name: Debounce_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Debounce_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Debounce_MODE_MASK));
}


/*******************************************************************************
* Function Name: Debounce_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Debounce_MOD_SRC & (uint8)(Debounce_MODE_MASK);
}


/*******************************************************************************
* Function Name: Debounce_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Debounce_GetDividerRegister();
    uint8 oldSrc = Debounce_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Debounce_MOD_SRC |= CYCLK_SSS;
        Debounce_MOD_SRC =
            (Debounce_MOD_SRC & (uint8)(~Debounce_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Debounce_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Debounce_MOD_SRC & Debounce_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Debounce_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Debounce_PHASE = clkPhase & Debounce_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Debounce_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Debounce_PHASE & Debounce_PHASE_MASK;
}
