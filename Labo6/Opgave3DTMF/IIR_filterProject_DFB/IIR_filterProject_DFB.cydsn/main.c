...
#define USE_FILTER_RECHTS 1
// LCD-tekst
#define TITEL1 "bandsperfilter "
#define TITEL2 "2e orde 100Hz & 300Hz"
...
void berekenMetSamples(){
    ...
    maxL = MAX(ABS(sampleUit[LINKS]) , maxL);
    maxR = MAX(ABS(sampleUit[RECHTS]), maxR);
    if (maxL > 2000 && maxR > 2000) {
        LEDs_Write(1);
    } else {
        LEDs_Write(0);
    }
